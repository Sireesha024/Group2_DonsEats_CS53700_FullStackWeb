...
it("should successfully delete a menu item", (done) => {
    chai
      .request(app)
      .delete("/api/deleteMenuItem")
      .send({ item: "Ham & Swiss", category: "Egg Sandwiches", subcategory: "Classics" })
      .end((err, res) => {
        if (err) return done(err); // Handle errors directly

        expect(res).to.have.status(200);
        expect(res.body).to.be.an("object");
        expect(res.body.message).to.equal("Menu item deleted successfully.");
        // expect(deleteStub.calledOnce).to.be.true; // Ensure Firestore delete was called
    //    sinon.assert.calledOnce(deleteStub);
        done();
      });
  });

  it("should handle menu item not found error", (done) => {
    // deleteStub.returns({get: sinon.stub().resolves({ empty: true })}); // Stub returns empty snapshot
    chai
      .request(app)
      .delete("/api/deleteMenuItem")
      .send({ item: "NonExistentItem", category: "Test Category", subcategory: "Test Subcategory" })
      .end((err, res) => {
        if (err) return done(err); // Handle errors directly
        expect(res).to.have.status(404);
        expect(res.body).to.be.an("object");
        expect(res.body.error).to.equal("Menu item not found.");
        done();
      });
  });
