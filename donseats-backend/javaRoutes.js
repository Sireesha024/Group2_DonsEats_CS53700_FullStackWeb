...
router.get("/menuItems", async (req, res) => {
    // Async route handler
    console.log("Received request for /api/javaSpotMenuItems");
    try {
      const menuItemsCollection = db.collection("javaSpotMenuItems"); // Use db directly
  ...
  router.post("/addMenuItem", upload.single("image"), async (req, res) => {
    try {
      ...
      stream.on("finish", async () => {
        ...
        const newMenuItemDocumentRef = await db.collection("javaSpotMenuItems").add({
          category,
          subcategory,
          title,
          price: price + " USD",
  ...
  router.get("/menuItems/updates", async (req, res) => {
    ...
    const unsubscribe = db.collection("javaSpotMenuItems").onSnapshot((snapshot) => {
  ...
  router.post("/requestNewDish", async (req, res) => {
    try {
      ...
      const requestDocRef = await db.collection("javaSpot").add({
  ...
  router.get("/javaSpot", async (req, res) => {
    // New route
    try {
      const einsteinBagelsRef = db.collection("javaSpot");
  ...
  router.post('/javaOrder', async (req, res) => {
    try {
      ...
      const orderDocRef = await db.collection('javaOrder').doc(orderId).set({ // Use doc(orderId) to set ID. Use set instead of add to manually set the doc ID
  ...
  router.get('/javaMenuItems', async (req, res) => {
    try {
      const bonsMenuItemsRef = db.collection('javaSpotMenuItems'); // Assuming you have a 'bonsMenuItems' collection in Firestore. You might want to rename this collection.
  ...
  router.delete('/deleteMenuItem', async (req, res) => {
    try {
      ...
      //  console.log(req.body);
        ...
        const snapshot = await db.collection('javaSpotMenuItems')
            .where('title', '==', item)
